  ...
  return (
    <div>
      <div className="admin-page-heading">
        ...
        <span className={error ? "sync-badge offline" : "sync-badge"}><i />{syncing ? "Menyinkronkan..." : error ? "Gangguan koneksi" : "Tersinkron"}</span>
      </div>
      ...
      <section className="admin-section-card">
        <div className="card-heading"><div><h2>Aktivitas terbaru</h2><p>Perubahan terbaru pada sistem.</p></div><button className="btn btn-soft btn-small" onClick={load}>Segarkan</button></div>
        {!data?.aktivitasTerbaru?.length ? <div className="empty-state">Belum ada aktivitas.</div> : (
          <div className="activity-list">{data.aktivitasTerbaru.map((row, i) => <div className="activity-item" key={String(row.ID || i)}><span className="activity-dot" /><div><strong>{String(row.Aktivitas || "Aktivitas")}</strong><p>{String(row.Keterangan || row.Jenis_Data || "")}</p></div><time>{String(row.Tanggal || "")} {String(row.Jam || "")}</time></div>)}</div>
        )}
      </section>
    </div>
  );
